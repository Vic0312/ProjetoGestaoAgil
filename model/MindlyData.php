...
final class MindlyData {
    ...
    public function specialtyOptions(): array { return query('SELECT id,nome FROM especialidades ORDER BY nome')->fetchAll(); }
    public function selectedSpecialties(): array { $this->only('psicologo');return query('SELECT especialidade_id FROM psicologo_especialidades WHERE psicologo_id=?',[$this->id])->fetchAll(PDO::FETCH_COLUMN); }
    public function statusHistory(int $consultation): array {
        [$where,$params]=$this->scope();array_unshift($params,$consultation);
