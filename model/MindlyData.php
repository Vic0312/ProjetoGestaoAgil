...
final class MindlyData {
    private int $id;
    private string $role;
    public function __construct(array $user) { $this->id=(int)$user['id']; $this->role=$user['papel']; }
    private function only(string $role): void {
        if ($this->role !== $role) throw new LogicException('Perfil não autorizado.');
    }
    private function scope(string $alias='c'): array {
        if ($this->role==='admin') return ['1=1', []];
        $column=['paciente'=>'paciente_id','psicologo'=>'psicologo_id'][$this->role] ?? null;
        if (!$column) throw new LogicException('Perfil inválido.');
        return ["$alias.$column=?",[$this->id]];
    }
    public function consultations(?int $id=null): array {
        [$where,$params]=$this->scope();
        if ($id!==null) { $where.=' AND c.id=?'; $params[]=$id; }
        return query("SELECT c.id,c.paciente_id,c.psicologo_id,c.valor,c.modalidade,c.status,c.criado_em,c.iniciada_em,c.encerrada_em,
    ...
    public function profile(): array {
        if ($this->role==='paciente') return query('SELECT data_nascimento FROM pacientes WHERE usuario_id=?',[$this->id])->fetch() ?: [];
        $this->only('psicologo');
        return $this->professional($this->id, true) ?: [];
    }
    private function professionalSelect(): string {
        return "SELECT p.usuario_id,p.crp,p.area_atuacao,p.biografia,p.abordagem,p.valor_consulta,p.duracao_padrao_minutos,p.status_verificacao,
    ...
    public function professional(int $id, bool $own=false): ?array {
        if ($own) { $this->only('psicologo'); if ($id!==$this->id) throw new LogicException('Perfil não autorizado.'); }
        else $this->only('paciente');
    ...
    public function professionals(string $search='', string $specialty=''): array {
        ...
        if ($specialty!=='') { $where.=' AND EXISTS (SELECT 1 FROM psicologo_especialidades pe JOIN especialidades e ON e.id=pe.especialidade_id WHERE pe.psicologo_id=p.usuario_id AND e.nome=?)'; $params[]=$specialty; }
        return query($this->professionalSelect().$where.' ORDER BY nome_profissional,p.usuario_id',$params)->fetchAll();
    }
    public function specialties(): array { return query('SELECT nome FROM especialidades ORDER BY nome')->fetchAll(PDO::FETCH_COLUMN); }
    public function slots(int $professional, ?int $limit=null): array {
        if ($this->role==='psicologo' && $professional!==$this->id) throw new LogicException('Agenda não autorizada.');
        if (!in_array($this->role,['paciente','psicologo'],true)) throw new LogicException('Perfil inválido.');
    ...
    public function availability(): array {
        $this->only('psicologo');
        return [
            'weekly'=>query('SELECT dia_semana,hora_inicio,hora_fim,ativa FROM disponibilidades_semanais WHERE psicologo_id=? ORDER BY dia_semana,hora_inicio',[$this->id])->fetchAll(),
            'blocks'=>query('SELECT inicio_em,fim_em,motivo FROM bloqueios_agenda WHERE psicologo_id=? AND fim_em>UTC_TIMESTAMP() ORDER BY inicio_em',[$this->id])->fetchAll(),
        ];
    }
    public function patients(string $search=''): array {
        $this->only('psicologo');
        return query("SELECT u.id,u.nome,
            (SELECT MIN(c.criado_em) FROM consultas c WHERE c.paciente_id=u.id AND c.psicologo_id=? AND c.status IN ('confirmada','em_andamento','concluida')) desde,
            ...
            (EXISTS (SELECT 1 FROM consultas c WHERE c.paciente_id=u.id AND c.psicologo_id=? AND c.status IN ('confirmada','em_andamento','concluida'))
            OR EXISTS (SELECT 1 FROM prontuarios pr WHERE pr.paciente_id=u.id AND pr.psicologo_id=?)) ORDER BY u.nome,u.id",
            [$this->id,$this->id,$this->id,'%'.$search.'%',$this->id,$this->id])->fetchAll();
    }
    ...
    public function notifications(): array {
        return query('SELECT titulo,mensagem,criado_em FROM notificacoes WHERE usuario_id=? AND lida_em IS NULL ORDER BY criado_em DESC,id DESC',[$this->id])->fetchAll();
    }
    public function users(string $search='', string $role=''): array {
        $this->only('admin');
        $params=['%'.$search.'%','%'.$search.'%'];$where='(u.nome LIKE ? OR u.email LIKE ?)';
        if(in_array($role,['paciente','psicologo','admin'],true)) { $where.=' AND u.papel=?';$params[]=$role; }
