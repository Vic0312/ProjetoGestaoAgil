<?php
function dateLabel(?string $value, string $format = 'd/m/Y H:i'): string
{
    return $value ? mindlyTime($value)->format($format) : '—';
}
...
function emptyState(string $message): void
{
    echo '<p class="estado-vazio" role="status">' . e($message) . '</p>';
}
function unavailable(string $feature): void
{
    echo '<p class="aviso-disponibilidade">' . e($feature) . ' ainda não disponível.</p>';
}
function rating(array $p): string
{
    return $p['nota'] === null ? 'Sem avaliações' : number_format((float) $p['nota'], 1, ',', '.');
}
function publicLink(string $page, $id, string $key = 'psicologo_id'): string
{
    return $page . '?' . $key . '=' . rawurlencode((string) $id);
}
...
{
    foreach (explode(', ', $p['especialidades'] ?? '') as $tag)
        if ($tag !== '')
            echo '<span class="tag">' . e($tag) . '</span>';
}
...
function avatar(array $user, string $class = 'avatar-foto'): void
{
    ...
    if (!empty($user['avatar_url']) && preg_match('/^[a-f0-9]{48}\.(jpg|png)$/D', $user['avatar_url'])) {
        ...
        echo '<span class="' . e($class) . '">' . e(initials($name)) . '</span>';
}
