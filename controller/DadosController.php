...
function completed(array $rows): array
{
    return array_values(array_filter($rows, fn($c) => $c['status'] === 'concluida'));
}
function pageData(string $page, array $user): array
{
    ...
    if (in_array($page, ['dashboardPaciente.php', 'dashboardPsicologo.php', 'dashboardAdmin.php', 'minhasConsultas.php', 'agendaPsicologo.php', 'atendimentosAdmin.php', 'relatoriosAdmin.php', 'pagamentosPaciente.php'], true)) {
        ...
        $d['proximas'] = upcoming($d['consultas']);
        $d['realizadas'] = completed($d['consultas']);
    ...
    switch ($page) {
        case 'dashboardPsicologo.php':
            ...
            break;
        case 'perfilPaciente.php':
        case 'perfilProfissional.php':
            $d['perfil'] = $model->profile();
            if ($page === 'perfilProfissional.php') {
                $d['opcoesEspecialidades'] = $model->specialtyOptions();
                $d['selecionadas'] = $model->selectedSpecialties();
            }
            break;
        case 'buscarPsicologos.php':
            $d['especialidade'] = requestText('especialidade');
            $d['especialidades'] = $model->specialties();
            ...
            unset($professional);
            break;
        case 'perfilPsicologo.php':
        case 'agendarConsulta.php':
            ...
            if ($id !== null) {
                ...
                if (!$d['profissional']) {
                    http_response_code(404);
                    exit('Profissional não encontrado.');
                }
        ...
        case 'disponibilidadePsicologo.php':
            $d['perfil'] = $model->profile();
            $d['disponibilidade'] = $model->availability();
            $d['horarios'] = $model->slots((int) $user['id']);
            break;
        case 'pacientesPsicologo.php':
        case 'prontuarioPsicologo.php':
            ...
            if (in_array($page, ['prontuarioPsicologo.php', 'pacientesPsicologo.php'], true)) {
                ...
                if ($id !== null) {
                    ...
                    if ($page === 'prontuarioPsicologo.php') {
                        ...
                        $d['contextoClinico'] = $model->clinicalContext($id);
                        $d['versoes'] = $model->recordVersions($id);
                    }
                }
            }
            break;
        case 'prontuarioPaciente.php':
            $d['historico'] = $model->sharedHistory();
            break;
        ...
        case 'salaAtendimentoPsicologo.php':
        ...
        case 'financeiroPsicologo.php':
            ...
            $d['consultas'] = $model->consultations();
            break;
        case 'notificacoesPsicologo.php':
            $d['avisos'] = $model->allNotifications();
            break;
        case 'usuariosAdmin.php':
            $d['filtroPerfil'] = requestText('perfil');
            $d['usuarios'] = $model->users($d['busca'], $d['filtroPerfil']);
            break;
        case 'dashboardAdmin.php':
        case 'relatoriosAdmin.php':
    ...
    if ($page === 'agendaPsicologo.php') {
        ...
        if (!$date || ($start && $date->format('Y-m-d') !== $start)) {
...
{
    try {
        return pageData($page, $user);
    } catch (Throwable $error) {
        error_log('Mindly: falha ao consultar dados da página (' . get_class($error) . ').');
        http_response_code(503);
        exit('Não foi possível carregar os dados. Tente novamente mais tarde.');
    }
}
