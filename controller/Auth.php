...
function showMessage(): void
{
    if (isset($_SESSION['flash'])) {
        echo '<p class="mensagem-formulario" role="alert">' . e($_SESSION['flash']) . '</p>';
...
function checkPost(): void
{
    ...
    if (!hash_equals(csrf(), input('csrf'))) {
...
function accountProblem(array $user): ?string
{
    ...
    if ($user['papel'] === 'psicologo' && $user['status_verificacao'] !== 'aprovado')
        return $user['status_verificacao'] === 'pendente' ? 'Cadastro recebido e aguardando validação profissional.' : 'Cadastro profissional não aprovado. Entre em contato com o suporte.';
    if ($user['status'] !== 'ativo')
        return 'Conta indisponível para acesso. Entre em contato com o suporte.';
    return null;
}
...
function requireRole(string $role): array
{
    ...
    try {
        $user = accountById($_SESSION['user_id']);
    } catch (Throwable $ex) {
    ...
    if ($user['papel'] !== $role) {
        http_response_code(403);
        exit('Acesso não autorizado para este perfil.');
    }
    return $user;
}
function validatePassword(string $password, string $confirm): void
{
    ...
    if (strlen($password) > 72 || strpos($password, "\0") !== false)
        throw new DomainException('A senha excede o limite de 72 bytes ou contém caracteres inválidos.');
    if ($password !== $confirm)
