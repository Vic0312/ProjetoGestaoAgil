...
class Controlador
{
    public static function cadastro(): void
    {
        ...
        if (input('termos') !== 'on')
            throw new DomainException('Aceite os Termos de Uso e a Política de Privacidade.');
        if (!in_array($role, ['paciente', 'psicologo'], true))
            throw new DomainException('Selecione um perfil válido.');
        $crp = trim(input('crp'));
        $professional = trim(input('nome_profissional'));
        $area = trim(input('area_atuacao'));
        if ($role === 'psicologo') {
            if (!preg_match('~^[0-9]{2}/[0-9]{4,7}$~D', $crp))
                throw new DomainException('Informe o CRP no formato 00/00000. O registro será verificado separadamente.');
            if (mb_strlen($professional) > 150 || mb_strlen($area) > 150)
                throw new DomainException('Os campos profissionais devem ter até 150 caracteres.');
        }
        $hash = password_hash(input('senha'), PASSWORD_DEFAULT);
        db()->beginTransaction();
        try {
            if (accountByEmail($email))
                throw new DomainException('Não foi possível cadastrar com este e-mail. Tente entrar ou recuperar a senha.');
            if ($role === 'psicologo' && query('SELECT usuario_id FROM psicologos WHERE crp=?', [$crp])->fetch())
                throw new DomainException('Não foi possível cadastrar com este CRP. Entre em contato com o suporte.');
            query('INSERT INTO usuarios (nome,email,senha_hash,papel,status) VALUES (?,?,?,?,?)', [$name, $email, $hash, $role, $role === 'paciente' ? 'ativo' : 'pendente']);
            $id = db()->lastInsertId();
            if ($role === 'paciente')
                query('INSERT INTO pacientes (usuario_id) VALUES (?)', [$id]);
            else
                query('INSERT INTO psicologos (usuario_id,crp,nome_profissional,area_atuacao,status_verificacao) VALUES (?,?,?,?,?)', [$id, $crp, $professional ?: null, $area ?: null, 'pendente']);
            foreach (['termos_uso', 'politica_privacidade'] as $type)
                query('INSERT INTO consentimentos (usuario_id,tipo,versao) VALUES (?,?,?)', [$id, $type, config()['terms_version']]);
            db()->commit();
        } catch (Throwable $ex) {
            if (db()->inTransaction())
                db()->rollBack();
            if ($ex instanceof PDOException && $ex->getCode() === '23000')
                throw new DomainException('E-mail ou CRP indisponível para cadastro.');
            throw $ex;
        }
        if ($role === 'paciente') {
            authenticate(accountById($id));
            redirect(dashboard($role));
        }
        unset($_SESSION['user_id'], $_SESSION['password_stamp']);
        flash('Cadastro recebido e aguardando validação profissional. A análise do CRP não é automática.');
    ...
    public static function recuperar(): void
    {
        ...
        $c = config();
        if (!$c['mail_enabled'])
            throw new DomainException('A recuperação por e-mail está indisponível no momento. Entre em contato com o suporte.');
        if (!filter_var($c['mail_from'], FILTER_VALIDATE_EMAIL) || preg_match('/[\r\n]/', $c['mail_from']) || !filter_var($c['app_url'], FILTER_VALIDATE_URL))
        ...
        flash('Se houver uma conta para este e-mail, as orientações serão encaminhadas.');
    ...
    public static function redefinir(): void
    {
        ...
        if (!preg_match('/^[a-f0-9]{64}$/D', $token))
            throw new DomainException('Link inválido ou expirado. Solicite novas orientações.');
        $hash = hash('sha256', $token);
        ...
        flash('Senha atualizada. Entre com sua nova senha.');
        redirect('view/login.php');
    }
}
