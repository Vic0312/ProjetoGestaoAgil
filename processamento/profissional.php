...
if(!in_array($action,$allowed,true)){http_response_code(400);exit('Operação inválida.');}
...
if(in_array($action,['registro','resumo'],true) && ctype_digit(input('paciente_id'))) $back.='?paciente_id='.input('paciente_id');
if($action==='agendar' && ctype_digit(input('psicologo_id'))) $back.='?psicologo_id='.input('psicologo_id');
if(in_array($action,['confirmar','cancelar','remarcar','iniciar','concluir','ausencia'],true) && ctype_digit(input('consulta_id'))) $back='view/'.($role==='psicologo'?'salaAtendimentoPsicologo.php':'salaAtendimento.php').'?consulta_id='.input('consulta_id');
try {$back=ProfissionalController::run($user,$action);unset($_SESSION['operacao_anterior']);}
catch(DomainException $error){flash($error->getMessage());
    ...
    $old=[];foreach($fields as $field)if(isset($_POST[$field]))$old[$field]=is_string($_POST[$field])?mb_substr($_POST[$field],0,10000):($field==='especialidades' && is_array($_POST[$field])?array_slice(array_filter($_POST[$field],'is_scalar'),0,50):'');
    $_SESSION['operacao_anterior']=['acao'=>$action,'dados'=>$old];
}
catch(Throwable $error){error_log('Mindly: falha na operação profissional ('.get_class($error).').');flash('Não foi possível concluir a operação. Tente novamente.');}
redirect($back);
